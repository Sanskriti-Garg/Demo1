...
class App:
    ...
    def create_widgets(self):
        ...
        save_btn = tk.Button(btn_frame, text="Save", command=self.save_schema, width=10, height=2)
        save_btn.pack(side=tk.LEFT, padx=5)

        generate_btn = tk.Button(btn_frame, text="Generate", command=self.generate_classes, width=10, height=2)
        generate_btn.pack(side=tk.LEFT, padx=5)

        reset_btn = tk.Button(btn_frame, text="Reset", command=self.reset_text, width=10, height=2)
        reset_btn.pack(side=tk.LEFT, padx=5)

        # Icons for Import, Edit, Link, Download
        import_icon = tk.PhotoImage(file="import_icon.png").subsample(3, 3)
        edit_icon = tk.PhotoImage(file="edit_icon.png").subsample(3, 3)
        link_icon = tk.PhotoImage(file="link_icon.png").subsample(3, 3)
        download_icon = tk.PhotoImage(file="download_icon.png").subsample(3, 3)

        icon_frame = tk.Frame(self.root)
        icon_frame.pack(side=tk.RIGHT, padx=5, pady=5)

        import_btn = tk.Button(icon_frame, image=import_icon, command=self.import_schema, width=40, height=40)
        import_btn.image = import_icon
        import_btn.pack(pady=5)

        edit_btn = tk.Button(icon_frame, image=edit_icon, command=self.edit_json, width=40, height=40)
        edit_btn.image = edit_icon
        edit_btn.pack(pady=5)

        link_btn = tk.Button(icon_frame, image=link_icon, command=self.add_link, width=40, height=40)
        link_btn.image = link_icon
        link_btn.pack(pady=5)

        download_btn = tk.Button(icon_frame, image=download_icon, command=self.download_schema, width=40, height=40)
