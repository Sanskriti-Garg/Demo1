...
class App:
    ...
    def create_widgets(self):
        ...
        # Yellow line below the red banner
        yellow_banner = tk.Label(self.root, bg="yellow", height=1)
        yellow_banner.pack(fill=tk.X)
        ...
        import_btn = tk.Button(btn_frame, text="Import", command=self.import_schema)
        import_btn.pack(side=tk.LEFT, padx=5)

        edit_btn = tk.Button(btn_frame, text="Edit", command=self.edit_json)
        edit_btn.pack(side=tk.LEFT, padx=5)

        link_btn = tk.Button(btn_frame, text="Add Link", command=self.add_link)
        link_btn.pack(side=tk.LEFT, padx=5)

        download_btn = tk.Button(btn_frame, text="Download", command=self.download_schema)
