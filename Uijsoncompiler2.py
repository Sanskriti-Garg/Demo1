...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.save_btn = tk.Button(btn_frame, text="Save", command=self.save_schema, width=12, height=2)
        self.save_btn.pack(side=tk.LEFT, padx=5)

        self.generate_btn = tk.Button(btn_frame, text="Generate", command=self.handle_generate, width=12, height=2)
    ...
    def handle_generate(self):
        if not self.is_schema_valid:
            messagebox.showwarning("Generate", "Schema is invalid. Please compile and fix errors before generating.")
            return
