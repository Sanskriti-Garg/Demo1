...
class StartPage(tk.Frame):
    ...
    def show_package_module_popup(self):
        ...
        popup.title("Package and Module Information")
        popup.geometry("300x200")

        tk.Label(popup, text="Package Name:").pack(pady=10)
        package_entry = tk.Entry(popup, width=30)
        package_entry.pack(pady=10)

        tk.Label(popup, text="Module:").pack(pady=10)
        module_entry = tk.Entry(popup, width=30)
        module_entry.pack(pady=10)

        btn_frame = tk.Frame(popup)
        btn_frame.pack(pady=10)

        submit_btn = tk.Button(btn_frame, text="Submit", command=lambda: self.submit_package_module(popup, package_entry.get(), module_entry.get()))
        submit_btn.pack(side=tk.LEFT, padx=10)

        cancel_btn = tk.Button(btn_frame, text="Cancel", command=popup.destroy)
        cancel_btn.pack(side=tk.LEFT, padx=10)

    def submit_package_module(self, popup, package_name, module):
        if not package_name or not module:
            messagebox.showwarning("Warning", "Package Name and Module cannot be empty.")
            return
        self.save_package_module_details(package_name, module)
        popup.destroy()
        self.start_generation()

    def save_package_module_details(self, package_name, module):
        details = {
            "package_name": package_name,
            "module": module
        }
        with open("input_detail.json", "w") as file:
