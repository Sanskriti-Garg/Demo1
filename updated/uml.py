...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.text_area = CustomText(text_frame, wrap=tk.WORD, width=70, height=40, bg="#1e1e1e", fg="#d4d4d4", insertbackground="white", yscrollcommand=self.text_scrollbar.set)
        ...
        self.uml_label = ttk.Label(right_frame, background="#fff")
        self.uml_label.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
    ...
    def check_and_generate(self):
        if not self.is_schema_valid:
            messagebox.showwarning("Warning", "First compile.")
            return
    ...
    def show_package_module_popup(self):
        def generate_with_popup():
            package = package_entry.get()
            module = module_entry.get()
            popup.destroy()
            self.generate_classes(package, module)

        popup = tk.Toplevel(self)
        popup.title("Package and Module")
        popup.geometry("300x200")

        tk.Label(popup, text="Enter Package Name:").pack(pady=5)
        package_entry = tk.Entry(popup)
        package_entry.pack(pady=5)

        tk.Label(popup, text="Enter Module Name:").pack(pady=5)
        module_entry = tk.Entry(popup)
        module_entry.pack(pady=5)

        generate_button = tk.Button(popup, text="Generate", command=generate_with_popup)
        generate_button.pack(pady=10)

    def generate_classes(self, package_name, module_name):
        self.loading_label.pack(expand=True)
        self.progress_bar.pack(fill=tk.X, pady=(10, 0))
        self.progress_bar.start()

        threading.Thread(target=self._generate_classes_thread, args=(package_name, module_name)).start()

    def _generate_classes_thread(self, package_name, module_name):
        time.sleep(2)
        ...
        output = f"Generated {module_name}.java in package {package_name}."
    ...
    def reset_text(self):
        ...
        self.uml_label.config(image='')
        self.uml_label.pack_forget()
    ...
    def compile_schema(self):
        ...
        try:
            ...
            uml_image = self.generate_uml(schema)
            self.display_uml(uml_image)

        except json.JSONDecodeError as e:
            messagebox.showerror("JSON Error", f"Invalid JSON: {str(e)}")
        except ValidationError as e:
            messagebox.showerror("Validation Error", f"JSON schema validation error: {str(e)}")
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {str(e)}")

    def generate_uml(self, schema):
        uml_code = "@startuml\n"
        # Generate UML code from the schema
        # For example, adding classes, relationships based on the schema
        uml_code += "class MyClass {\n  +attribute1 : String\n}\n"
        uml_code += "@enduml"

        plantuml = PlantUML(url="http://www.plantuml.com/plantuml/img/")
        uml_image = plantuml.processes(uml_code)
        return uml_image

    def display_uml(self, uml_image):
        self.uml_label.config(image='')
        self.uml_label.pack_forget()

        img = Image.open(uml_image)
        img = ImageTk.PhotoImage(img)
        self.uml_label.config(image=img)
        self.uml_label.image = img
        self.uml_label.pack(expand=True)
    ...
    def import_schema(self):
        ...
        if file_path:
            ...
            uml_image = self.generate_uml(json.loads(schema))
            self.display_uml(uml_image)
    ...
    def download_schema(self):
        ...
        if save_path:
            ...
            messagebox.showinfo("Download", f"Schema downloaded to {save_path}")
