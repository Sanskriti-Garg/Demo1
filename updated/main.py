...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.initial_label = ttk.Label(right_frame, text="Output Screen", font=LARGEFONT, background="#f0f0f0")
        self.initial_label.pack(expand=True)

        self.loading_label = ttk.Label(right_frame, text="Generating Java classes...", font=LARGEFONT, background="#f0f0f0")
        self.progress_bar = ttk.Progressbar(right_frame, mode='indeterminate')

        # back_btn = ttk.Button(right_frame, text="Back", command=lambda: controller.show_frame(StartPage))
    ...
    def check_and_generate(self):
        schema_text = self.text_area.get("1.0", tk.END).strip()
        if schema_text == "Imported JSON will be shown here." or not schema_text:
            messagebox.showwarning("Warning", "Nothing to generate.")
            return
