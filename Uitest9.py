...
class App:
    ...
    def create_widgets(self):
        ...
        # Main frame for text area and icons
        ...
        self.text_area.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        ...
        icon_frame.grid(row=0, column=1, sticky="ns", padx=(5, 0))
    ...
    def switch_to_loading_screen(self):
        ...
        back_btn = tk.Button(self.main_frame, text="Back", command=self.switch_back_from_loading_screen, width=12, height=2)
    ...
    def switch_back_from_loading_screen(self):
        # Clear main frame
        for widget in self.main_frame.winfo_children():
            widget.destroy()

        # Re-create the original widgets
        self.create_widgets()

        # Restore the saved state of the text area and the import state
        self.text_area.config(state=tk.NORMAL)
        self.text_area.delete("1.0", tk.END)
        if self.schema:
            self.text_area.insert(tk.END, self.schema)
        self.text_area.config(state=tk.DISABLED if not self.json_loaded else tk.NORMAL)

    def reset_text(self):
        self.text_area.config(state=tk.NORMAL)
        self.text_area.delete("1.0", tk.END)
        self.text_area.config(state=tk.DISABLED)
        self.json_loaded = False  # Set json_loaded to False
