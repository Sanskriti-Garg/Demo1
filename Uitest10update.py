...
class App:
    ...
    def create_rounded_button_image(self, text, width, height, corner_radius, color):
    ...
    def create_widgets(self):
        ...
        yellow_banner = tk.Label(self.root, bg="yellow", height=1)
    ...
    def create_main_screen(self):
        ...
        self.text_area = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=60, height=20)
        ...
        # Buttons for Save, Generate, Reset below the JSON area
        btn_frame = tk.Frame(frame)
        btn_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=5, padx=10)

        self.rounded_save_img = self.create_rounded_button_image("Save", 100, 40, 10, "green")
        self.rounded_generate_img = self.create_rounded_button_image("Generate", 100, 40, 10, "blue")
        self.rounded_reset_img = self.create_rounded_button_image("Reset", 100, 40, 10, "red")

        save_btn = tk.Button(btn_frame, text="Save", image=self.rounded_save_img, compound=tk.CENTER, command=self.save_schema, width=100, height=40, borderwidth=0)
        save_btn.pack(side=tk.LEFT, padx=5)

        generate_btn = tk.Button(btn_frame, text="Generate", image=self.rounded_generate_img, compound=tk.CENTER, command=self.start_generation, width=100, height=40, borderwidth=0)
        generate_btn.pack(side=tk.LEFT, padx=5)

        reset_btn = tk.Button(btn_frame, text="Reset", image=self.rounded_reset_img, compound=tk.CENTER, command=self.reset_text, width=100, height=40, borderwidth=0)
        reset_btn.pack(side=tk.LEFT, padx=5)

    def create_loading_screen(self):
        frame = tk.Frame(self.root)
        self.frames["LoadingScreen"] = frame

        self.loading_label = tk.Label(frame, text="Generating Java classes...", font=("Helvetica", 16))
        ...
        back_btn = tk.Button(frame, text="Back", command=lambda: self.show_frame("MainScreen"), width=12, height=2)
...
if __name__ == "__main__":
    ...
    # Pack all frames on the root window
